...
public class HillCipher {
	...
	public static void main(String[] args) throws IOException{
		...
		Matrix<Rational> encryptionKey = loadKeyFromFile(new File(keyFile), radix);
		int[] intSequence = loadIntSequenceFromFile(plainTextFileName);
		int paddingNumber =  paddingInfo(intSequence, blocksize);
		int[] paddedintSequence = padIntSequence(intSequence);
		Vector<Rational>[] vectors = divideIntSeqIntoVectors(paddedintSequence, blocksize);
		int[] encryptedIntSeq = encryptIntSequence(vectors, encryptionKey, radix);
		String encryptedIntSeqStr = convertIntArrToStr(encryptedIntSeq);
		System.out.println(encryptedIntSeqStr);
		writeCipherTextToFile(cipherFileName, encryptedIntSeqStr);
	}
	...
	private static int[] encryptIntSequence(Vector<Rational>[] vectors, Matrix<Rational> decipherKey,int radix) {
	...
	private static Matrix<Rational> loadKeyFromFile(File file, int mod) throws IOException {
		...
		Matrix<Rational> key = modMatrixToJscienceMatrix(modMatrix);
		return key;
	}
	...
	private static void writeCipherTextToFile(String plainTextFileName, String cipherText) throws IOException {
