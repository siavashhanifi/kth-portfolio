...
public class ConcHTTPAsk implements Runnable{
	...
	private ConcHTTPAsk(Socket connectionSocket) {
		this.conSock = connectionSocket;
	}
	...
	public static void main( String[] args) throws IOException{ 
		...
		while(true) { 
			...
			(new Thread(new ConcHTTPAsk(connectionSocket))).start();
	
		} 

	}

	private static String constructHTTPResponse(String HTTPGETRequest){// throws MalformedURLException {
	...
	public void run() {// throws MalformedURLException 
		...
		try {
		HTTPGETRequest = readFromClient(this.conSock);
		}
		...
		try {
			sendToClient(this.conSock, HTTPResponse);
			conSock.close();
			}
		catch(IOException ioEx){
			return;
